...
矢印キーまたは `j`/`k` で選択、`Enter` で確定、`q` または `Esc` で終了。選択中のセッションは行全体の背景色で表示します。

```
  agres 0.4.0  [all projects]
  /projects/web-app

   2026-07-22 06:30:00  [opencode]  [web-app]  ログインバグ修正  opencode
   2026-07-21 22:15:00  [claude]    [api]      認証モジュールリファクタ  claude
   2026-07-21 22:15:00  [agy]       [weather]  天気を確認
   2026-07-20 14:00:00  [aider]     [current]  ユニットテスト追加
